...
def match_implicit_labels(
    label_memo: TransactionLabel.true_memo, candidate_memo: PlaidTransaction
) -> bool:
    return re.sub("[^a-z0-9]", "", label_memo.lower()) == re.sub(
        "[^a-z0-9]", "", label_memo.lower()
    )
...
class User:
    ...
    explicit_labels: DataFrame[TransactionLabel]
    implicit_labels: DataFrame[TransactionLabel] = has_many(match_implicit_labels)
